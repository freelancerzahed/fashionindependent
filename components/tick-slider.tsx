...
interface TickSliderProps {
  ...
  label: string
  disabled?: boolean
  displayValue?: number // Value to display (can be different from the slider value)
}
...
export const TickSlider = memo(function TickSlider({
  ...
  // Calculate thumb position based on raw value (for interaction)
  useEffect(() => {
    const pct = ((value - min) / (max - min)) * 100
    setThumbPosition(Number.isFinite(pct) ? Math.min(100, Math.max(0, pct)) : 0)
  }, [value, min, max])
  
  // Use displayValue if provided, otherwise use raw value
  const displayVal = displayValue ?? value

  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => onChange(Number.parseFloat(e.target.value)),
    [onChange],
  )
  ...
  return (
    <div className="slider-container">
      <div className="slider-track-container" ref={trackRef}>
        ...
        .slider-value-display { display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem; padding: 0 0.25rem; }
        .slider-value-display span { transition: all 0.2s ease; }

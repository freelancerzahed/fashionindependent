...
export default function SettingsPage() {
  const { fetchProfile } = useProfileImage()
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        console.log('[Personal-Info] Starting profile fetch...')
        const profile = await fetchProfile()
        console.log('[Personal-Info] Profile fetched:', profile)
        ...
        if (profile) {
          ...
          // Set profile image - convert path to /api/storage format
          if (profile.avatar_path) {
            const imageUrl = `/api/storage/${profile.avatar_path}`
            console.log('[Personal-Info] Setting image URL:', imageUrl)
            setProfileImage(imageUrl)
          } else if (profile.avatar) {
            // Fallback to avatar field if avatar_path is not available
            console.log('[Personal-Info] Using avatar field:', profile.avatar)
            setProfileImage(profile.avatar)
          } else {
            console.log('[Personal-Info] No avatar or avatar_path found in profile')
          }
        }
      } catch (error) {
        console.error("[Personal-Info] Failed to load profile:", error)
        // Keep using placeholder if fetch fails
      }
    }
  ...
  const handleImageUploadSuccess = (imageUrl: string) => {
    setProfileImage(imageUrl || "/placeholder.svg?height=80&width=80&text=User")
  }
      ...
      <Tabs defaultValue="profile" className="w-full">
        ...
        <TabsContent value="profile" className="mt-6">
          <div className="space-y-6">
            ...
            {/* Debug Info */}
            {profileImage && profileImage !== "/placeholder.svg?height=80&width=80&text=User" && (
              <Card className="bg-blue-50 border-blue-200">
                <CardContent className="pt-6">
                  <p className="text-xs text-blue-700 font-mono break-all">
                    <strong>Debug - Image URL:</strong> {profileImage}
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-primary-600">Personal Information</CardTitle>

  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { success: false, message: "User parameter is required" },
        { status: 400 }
      );
    }
    ...
    const backendUrl = `${BACKEND_URL}/chat/messages/${user}?page=${page}`;
    console.log("Fetching messages from backend:", backendUrl);

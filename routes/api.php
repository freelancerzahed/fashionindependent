...
Route::options('{any}', function () {
    $origin = request()->header('Origin') ?? 'http://192.168.0.102:3000';
    // Allow requests from localhost:3000, 127.0.0.1:3000, and the configured IP
    $allowedOrigins = [
        'http://localhost:3000',
        'http://127.0.0.1:3000',
    ...
    $allowOrigin = in_array($origin, $allowedOrigins) ? $origin : 'http://192.168.0.102:3000';
...
})->where('any', '.*')->withoutMiddleware(['app_language']);
...
})->withoutMiddleware([\App\Http\Middleware\EnsureSystemKey::class]);
...
|--------------------------------------------------------------------------
*/
Route::group(['prefix' => 'v2/chat', 'middleware' => ['app_language']], function () {

    Route::middleware('auth:sanctum')->group(function () {

        Route::controller(MessageController::class)->group(function () {

            Route::get('messages/{user_name}', 'index');             // Get messages with a user
            Route::post('messages/send', 'store');                  // Send a message
            Route::get('contacts', 'getContacts');                 // Get all contacts
            Route::get('conversations', 'allConversations');       // Get all conversations
            Route::delete('messages/{id}', 'deleteMessage');       // Delete a message
            Route::delete('conversations/{id}', 'deleteConversation'); // Delete a conversation
            Route::post('conversations/{id}/read', 'markAsRead');  // Mark messages as read
        });
...
Route::group(['prefix' => 'v2/auth', 'middleware' => ['app_language']], function () {
    ...
    Route::middleware(['parse_bearer_token', 'auth:sanctum'])->group(function () {
        Route::get('test', [\App\Http\Controllers\Api\V2\DebugAuthController::class, 'test']);
    });
...
Route::middleware('auth:sanctum')->group(function () {
    Route::controller(AuthController::class)->group(function () {
        ...
        // Body data
        Route::get('body-data', 'body_data');
        Route::post('body-data/save', 'save_body_data');
        Route::put('body-data/update', 'update_body_data'); // update payload
    });
    ...
    Route::controller(UserProfileController::class)->prefix('profile')->group(function () {
        ...
        Route::post('image/upload', 'uploadProfileImage');
...
Route::group(['prefix' => 'v2/creator', 'middleware' => ['app_language']], function () {
    ...
    Route::middleware(['parse_bearer_token', 'auth:sanctum'])->group(function () {
        ...
        Route::controller(DocumentController::class)->group(function () {
            ...
            Route::get('documents/{type}', 'getByType');
            Route::post('documents/upload', 'upload');
...
// Campaign Routes (FashionIndependent)
Route::group(['prefix' => 'v2/campaign', 'middleware' => ['app_language']], function () {
    // Authenticated routes first (more specific patterns)
    Route::middleware(['parse_bearer_token', 'auth:sanctum'])->group(function () {
        Route::controller(CampaignController::class)->group(function () {
            Route::post('/', 'store');
            Route::post('{id}/upload-files', 'uploadFiles');
            Route::get('/', 'getCampaigns');
            Route::put('{id}', 'update');
            Route::post('{id}/submit', 'submitForReview');
            Route::post('{id}/launch', 'launch');
        });
    });
...
Route::group(['prefix' => 'v2/pledge', 'middleware' => ['app_language']], function () {
    Route::middleware(['parse_bearer_token', 'auth:sanctum'])->group(function () {
        Route::controller(PledgeController::class)->group(function () {
            Route::post('/', 'store');
            Route::post('{id}/upload-files', 'uploadFiles');
            Route::get('user', 'getUserPledges');
            Route::get('campaign/{id}', 'getCampaignPledges');
...
Route::group(['prefix' => 'v2/friend', 'middleware' => ['app_language']], function () {
    Route::middleware('auth:sanctum')->group(function () {
        Route::controller(FriendController::class)->group(function () {

            // 📨 Friend request actions
            Route::get('requests', 'friendRequests');                 // Get incoming requests
            Route::post('request/send', 'sendFriendRequest');         // Send friend request
            Route::post('request/{id}/accept', 'acceptFriendRequest'); // Accept friend request
            Route::post('request/{id}/decline', 'declineFriendRequest'); // Decline friend request

            // ❌ Remove or block
            Route::post('remove', 'removeFriend');                    // Unfriend
            Route::post('block/{id}', 'blockFriend');                 // Block user

            // 👥 Friends list
            Route::get('all', 'allFriends');                          // All (pending + accepted)
            Route::get('accepted', 'acceptedFriends');                // Only accepted friends

            // 🧠 Friend suggestions (NEW)
            Route::get('suggestions', 'suggestions');                 // Suggested friends
        });
    });
});
...
Route::group(['prefix' => 'v2', 'middleware' => ['app_language']], function () {
    ...
    Route::group(['middleware' => ['app_user_unbanned']], function () {
        ...
        Route::controller(ChatController::class)->group(function () {
            ...
            Route::post('chat/send-message-to-user', 'send_message_to_user')->middleware('auth:sanctum');
    ...
    Route::get('categories/menu', 'App\Http\Controllers\Api\V2\CategoryController@menu');

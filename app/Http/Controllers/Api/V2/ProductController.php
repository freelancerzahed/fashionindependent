...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        $products = Product::where('published', 1)
            ...
            ->select([
                'id', 'name', 'slug', 'unit_price', 'thumbnail_img',
                'current_stock', 'rating', 'num_of_sale', 'category_id', 
                'user_id', 'discount', 'discount_type', 'featured', 'published', 'approved',
        ...
        return response()
            ->json(new ProductMiniCollection($products))
            ->header('Cache-Control', 'public, max-age=3600, s-maxage=3600')
            ->header('ETag', md5(json_encode($products->items())));
    }
    ...
    public function product_details($slug, $user_id)
    {
        $product = Product::where('slug', $slug)
            ->with([
                'user:id,name',
                'user.shop:id,name,slug,logo',
                'stocks:id,product_id,qty,price,variant,image',
        ...
        if(get_setting('last_viewed_product_activation') == 1 && $user_id != null && $product->count() > 0){
    ...
    public function categoryProducts($slug, Request $request)
    {
        ...
        $products = $category->products()
            ->where('published', 1)
            ->where('approved', 1)
            ->with(['main_category:id,name,slug', 'user:id,name'])
            ->select([
                'id', 'name', 'slug', 'unit_price', 'thumbnail_img',
                'current_stock', 'rating', 'num_of_sale', 'category_id', 
                'user_id', 'discount', 'discount_type', 'featured', 'published', 'approved'
        ...
        $products = filter_products($products)->latest()->paginate($perPage);

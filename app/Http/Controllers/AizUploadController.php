        ...
        if ($request->hasFile('aiz_file')) {
            ...
            if (isset($type[$extension])) {
                ...
                if ($extension == 'svg') {
                    ...
                    file_put_contents(public_path($path), $cleanSVG);
                } else {
                    $request->file('aiz_file')->move(public_path('uploads/all'), $path);
                }
                ...
                if ($type[$extension] == 'image' && $extension != 'svg') {
                    ...
                    try {
                        $img = Image::make(public_path($path))->encode($extension, 75);
                        $height = $img->height();
                        $width = $img->width();
                ...
                if (env('FILESYSTEM_DRIVER') != 'local') {
                    Storage::disk(env('FILESYSTEM_DRIVER'))->put(
                        ...
                        [
                            'visibility' => 'public',
                            'ContentType' => $extension == 'svg' ? 'image/svg+xml' : mime_content_type(public_path($path))
                        ]
                    );
                    if ($arr[0] != 'updates') {

...
class SSLCommerz
{
    ...
    protected function sendRequest($data)
    {
        ...
        curl_setopt($handle, CURLOPT_POSTFIELDS, http_build_query($data)); // Secure encoding
        curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($handle, CURLOPT_TIMEOUT, 30); // Prevents hanging requests
        ...
        if ($code == 200 && !(curl_errno($handle))) {
            curl_close($handle);
            $this->sslc_data = json_decode($content, true);
            return $this;
        } else {
            $error_message = curl_error($handle);
            curl_close($handle);
            \Log::error("SSLCOMMERZ API Connection Failed: " . $error_message); // Log errors
            $this->error = "Failed to connect with SSLCOMMERZ API: " . $error_message;
            return false;
        }
    }

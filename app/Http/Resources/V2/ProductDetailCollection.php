    ...
    public function toArray($request)
    {
        return [
            'data' => $this->collection->filter(function($data) {
                ...
                $whole_sale = [];
                if (addon_is_activated('wholesale') && $data->stocks && $data->stocks->count() > 0 && $data->stocks->first()->wholesalePrices) {
                    $whole_sale =  ProductWholesaleResource::collection($data->stocks->first()->wholesalePrices);
                }
                
                $first_stock = $data->stocks && $data->stocks->count() > 0 ? $data->stocks->first() : null;
                ...
                return [
                    ...
                    'shop_name' => $data->added_by == 'admin' ? translate('In House Product') : ($data->user && $data->user->shop ? $data->user->shop->name : ''),
                    'shop_logo' => $data->added_by == 'admin' ? uploaded_asset(get_setting('header_logo')) : ($data->user && $data->user->shop ? uploaded_asset($data->user->shop->logo) : ""),
                    'photos' => $photos,
                    ...
                    'current_stock' => $first_stock ? (int)$first_stock->qty : 0,
                    'unit' => $data->unit ?? "",
